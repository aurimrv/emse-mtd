...
{ ponta_de_prova (29798, 1);
    return exp(-x*x / 2) / sqrt(2 * 3.14159265358979323846); ponta_de_prova (29798, 2);
} 
double phi(double x, double mu, double sigma) 
{ ponta_de_prova (29886, 1);
    return phi01((x - mu) / sigma) / sigma; ponta_de_prova (29886, 2);
} 
double Phi201(double z) 
{ 
double sum = 0.0, term = z; 
int i; ponta_de_prova (29980, 1);
    if (z < -8.0) {
        ponta_de_prova (29980, 2);
        return 0.0; }
    if  ( ponta_de_prova (29980, 3), (z > 8.0) ){
        ponta_de_prova (29980, 4);
        return 1.0; }
    for ( ponta_de_prova (29980, 5), i = 3; ponta_de_prova (29980, 6), sum + term != sum; ponta_de_prova (29980, 7), i += 2  )
        { 
            ponta_de_prova (29980, 8);
            sum = sum + term; 
            term = term * z * z / i; 
        } 
    ponta_de_prova (29980, 9);
    return 0.5 + sum * phi01(z); ponta_de_prova (29980, 10);
} 
double Phi2(double z, double mu, double sigma) 
{ ponta_de_prova (30253, 1);
    return Phi201((z - mu) / sigma); ponta_de_prova (30253, 2);
} 
double PhiInverse02(double y, double delta, double lo, double hi) 
{ 
double mid = lo + (hi - lo) / 2; ponta_de_prova (30341, 1);
    if (hi - lo < delta) {
        ponta_de_prova (30341, 2);
        return mid; }
    if  ( ponta_de_prova (30341, 3), (Phi201(mid) > y) ){
        ponta_de_prova (30341, 4);
        return PhiInverse02(y, delta, lo, mid); }
    else {
        ponta_de_prova (30341, 5);
        return PhiInverse02(y, delta, mid, hi); }ponta_de_prova (30341, 6);
} 
double PhiInverse(double y) 
{ ponta_de_prova (30599, 1);
    return PhiInverse02(y, .00000001, -8, 8); ponta_de_prova (30599, 2);
} 
void main(int argc, char *argv[]) 
{ ponta_de_prova (30675, 1);
    if ( strcmp("-", argv[1]) == 0 ) 
        { 
            ponta_de_prova (30675, 2);
            driver(atoi(argv[2]), argc, argv); 
        } 
    else 
        { 
            ponta_de_prova (30675, 3);
            driver(0, argc, argv); 
        } ponta_de_prova (30675, 4);
} 
