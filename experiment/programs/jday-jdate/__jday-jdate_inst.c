...
long c, ya, j; ponta_de_prova (8950, 1);
    if (m > 2) {
    ...
    return(j); ponta_de_prova (8950, 5);
} 
int *
jdate(j) 
long j; 
{ 
static int ret[4]; 
long d, m, y; ponta_de_prova (9225, 1);
    ret[3] = (j + 1L)%7L; 
    j -= 1721119L; 
    y = (4L * j - 1L)/146097L; 
    j = 4L * j - 1L - 146097L * y; 
    ...
    y = 100L * y + j; 
    if (m < 10) {
        ponta_de_prova (9225, 2);
        m += 3; }
    else 
        { 
            ponta_de_prova (9225, 3);
            m -= 9; 
            ++y; 
        } 
    ponta_de_prova (9225, 4);
    ret[0] = m; 
    ret[1] = d; 
    ret[2] = y; 
    return(ret); ponta_de_prova (9225, 5);
} 
int main(int argc, char *argv[]) 
{ 
long l; 
int *d; ponta_de_prova (9664, 1);
    if (argc > 2) 
        { 
            ponta_de_prova (9664, 2);
            l = jday(atoi(argv[1]), atoi(argv[2]), atoi(argv[3])); 
            printf("Julian day: %ld\n", l); 
        } 
    else 
        { 
            ponta_de_prova (9664, 3);
            l = atol(argv[1]); 
            d = jdate(l); 
            printf("Month: %d Day: %d Year: %d Week day: %d\n", d[0], d[1],d[2], d[3]); 
        } ponta_de_prova (9664, 4);
} 
