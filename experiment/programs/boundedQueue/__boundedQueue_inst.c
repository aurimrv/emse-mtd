...
{ 
BoundedQueue *q; ponta_de_prova (24744, 1);
    if (maxElements < 0) 
        { 
            ponta_de_prova (24744, 2);
            printf("Invalid size.\n"); 
            q = 0; 
            return q; ponta_de_prova (24744, 3);
        } 
    ponta_de_prova (24744, 4);
    ...
    q->front = 0; 
    q->back = 0; 
    return q; ponta_de_prova (24744, 5);
} 
void enqueue(BoundedQueue *q, int e) 
{ ponta_de_prova (25067, 1);
    if (q->size == q->capacity) 
        { 
            ponta_de_prova (25067, 2);
            printf("Queue is Full\n"); 
        } 
    else 
        { 
            ponta_de_prova (25067, 3);
            q->size = q->size + 1; 
            q->elements[q->back] = e; 
            q->back = (q->back + 1) % q->capacity; 
        } ponta_de_prova (25067, 4);
} 
int isFull(BoundedQueue *q) 
{ ponta_de_prova (25274, 1);
    return q->size == q->capacity; ponta_de_prova (25274, 2);
} 
int isEmpty(BoundedQueue *q) 
{ ponta_de_prova (25338, 1);
    return q->size == 0; ponta_de_prova (25338, 2);
} 
int dequeue(BoundedQueue *q) 
{ 
int r; ponta_de_prova (25393, 1);
    if (q->size == 0) 
        { 
            ponta_de_prova (25393, 2);
            printf("Queue is already empty!\n"); 
            return -1; ponta_de_prova (25393, 3);
        } 
    else 
        { 
            ponta_de_prova (25393, 4);
            q->size = q->size - 1; 
            r = q->elements[(q->front % q->capacity)]; 
            q->front = (q->front + 1) % q->capacity; 
            return r; ponta_de_prova (25393, 5);
        } ponta_de_prova (25393, 6);
} 
void printBoundedQueue(BoundedQueue *q) 
{ 
int i, e; ponta_de_prova (25646, 1);
    printf("["); 
    for ( i = 0; ponta_de_prova (25646, 2), i < q->size; ponta_de_prova (25646, 3), i++  )
        { 
            ponta_de_prova (25646, 4);
            e = q->elements[(q->front + i) % q->capacity]; 
            printf("%d", e); 
            if (i < q->size - 1) 
                { 
                    ponta_de_prova (25646, 5);
                    printf(", "); 
                } ponta_de_prova (25646, 6);
        } 
    ponta_de_prova (25646, 7);
    printf("]\n"); 
} 
int main (int argc, char *argv[]) 
{ ponta_de_prova (25880, 1);
    driver(argc, argv); 
    return 0; ponta_de_prova (25880, 2);
} 
