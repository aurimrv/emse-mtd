...
int cal (int month1, int day1, int month2,
                       int day2, int year) 
{ 
int numDays, i; ponta_de_prova (9016, 1);
    if (month2 == month1) {
        ponta_de_prova (9016, 2);
        numDays = day2 - day1; }
    else 
        { 
        int m4 = year % 4; 
        int m100 = year % 100; 
        int m400 = year % 400; 
            if  ( ponta_de_prova (9016, 3), ((m4 != 0) || ((m100 == 0) && (m400 != 0))) ){
                ponta_de_prova (9016, 4);
                daysIn[2] = 28; }
            else {
                ponta_de_prova (9016, 5);
                daysIn[2] = 29; }
            ponta_de_prova (9016, 6);
            numDays = day2 + (daysIn[month1] - day1); 
            for ( i = month1 + 1; ponta_de_prova (9016, 7), i <= month2-1; ponta_de_prova (9016, 8), i++  ){
                ponta_de_prova (9016, 9);
                numDays = daysIn[i] + numDays; }ponta_de_prova (9016, 10);
        } 
    ponta_de_prova (9016, 11);
    return (numDays); ponta_de_prova (9016, 12);
} 
void main (int argc, char *argv[]) 
{ 
int month1, day1, month2, day2, year, T; ponta_de_prova (9552, 1);
    month1 = atoi(argv[1]); 
    day1 = atoi(argv[2]); 
    month2 = atoi(argv[3]); 
    day2 = atoi(argv[4]); 
