...
{ 
int min, indexOfMin, i; ponta_de_prova (20871, 1);
    if (low < high) 
        { 
            ponta_de_prova (20871, 2);
            min = list[low]; 
            indexOfMin = low; 
            for ( i = low + 1; ponta_de_prova (20871, 3), i <= high; ponta_de_prova (20871, 4), i++  )
                { 
                    if  ( ponta_de_prova (20871, 5), (list[i] < min) )
                        { 
                            ponta_de_prova (20871, 6);
                            min = list[i]; 
                            indexOfMin = i; 
                        } ponta_de_prova (20871, 7);
                } 
            ponta_de_prova (20871, 8);
            list[indexOfMin] = list[low]; 
            list[low] = min; 
            sort(list, low + 1, high); 
        } ponta_de_prova (20871, 9);
} 
double list[100]; 
void main(int argc, char *argv[]) 
{ 
int i; ponta_de_prova (21257, 1);
    for ( i = 1; ponta_de_prova (21257, 2), i < argc; ponta_de_prova (21257, 3), i++  ){
        ponta_de_prova (21257, 4);
        list[i-1] = atof(argv[i]); }
    ponta_de_prova (21257, 5);
    sort(list, 0, i - 2); 
    printf("******** Sorted numbers: \n"); 
    for ( i = 0; ponta_de_prova (21257, 6), i < argc-1; ponta_de_prova (21257, 7), i++  ){
        ponta_de_prova (21257, 8);
        printf("%.lf ",list[i]); }
    ponta_de_prova (21257, 9);
    printf("\n"); 
} 
