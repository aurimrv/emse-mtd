...
long	f[MAXTBL];		/* frequency tallies */
int	r;			/* # of rows */
int	c;			/* # of columns */
...
dispatch()
	...
	while ( fgets( line, MAXLINE, mystdin ) != NULL )	/* start new table */
		{
...
InfoTbl( r, c, f, pdf )
	...
	int	j;		/* column index */
	double		N;		/* (double)n */
	double		info = 0;		/* accumulates information measure */
	double		*xi;		/* row sums */
	double		*xj;		/* col sums */
