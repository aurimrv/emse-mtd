...
int rtnIndex = -1; 
int isPat = 0; 
int iPat = 0; ponta_de_prova (12740, 1);
    
    while  ( ponta_de_prova (12740, 2), (isPat == 0 && (iSub + patternLen - 1 < subjectLen)) )
        { 
            if  ( ponta_de_prova (12740, 3), (subject[iSub] == pattern[0]) )
                { 
                    ponta_de_prova (12740, 4);
                    rtnIndex = iSub; 
                    isPat = 1; 
                    for ( iPat = 1; ponta_de_prova (12740, 5), iPat < patternLen; ponta_de_prova (12740, 6), iPat++  )
                        { 
                            if  ( ponta_de_prova (12740, 7), (subject[iSub + iPat] != pattern[iPat]) )
                                { 
                                    ponta_de_prova (12740, 8);
                                    rtnIndex = -1; 
                                    isPat = 0; 
                                    break; ponta_de_prova (12740, 9);
                                } ponta_de_prova (12740, 10);
                        } ponta_de_prova (12740, 11);
                } 
            ponta_de_prova (12740, 12);
            iSub++; 
        } 
    ponta_de_prova (12740, 13);
    return rtnIndex; ponta_de_prova (12740, 14);
} 
int main(int argc, char* argv[]) 
{ 
int n = 0; ponta_de_prova (13214, 1);
    if (argc == 2) 
        { 
            ponta_de_prova (13214, 2);
            pattern[0] = '\0'; 
        } 
    else 
        { 
            ponta_de_prova (13214, 3);
            strcpy(pattern, argv[2]); 
        } 
    ponta_de_prova (13214, 4);
    strcpy(subject, argv[1]); 
    if ((n = pat(subject, strlen(subject), pattern, strlen(pattern))) == -1) 
        { 
            ponta_de_prova (13214, 5);
            printf("Pattern string is not a substring of the subject string\n"); 
        } 
    else 
        { 
            ponta_de_prova (13214, 6);
            printf("Pattern string begins at the character %i\n", n); 
        } 
    ponta_de_prova (13214, 7);
    return 0; ponta_de_prova (13214, 8);
} 
